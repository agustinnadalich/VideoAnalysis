    ...
    try:
        ...
        for ev in event_dicts:
            ...
            if team_name is None:
                ...
                # Inicializar perfil
        profile = None
        print(f"🔍 DEBUG: Inicializando perfil para match {match_id}")
        ...
        try:
            ...
            print(f"🔍 DEBUG: Valores del match - kick_off_1: {kick_off_1}, end_1: {end_1}")
            if kick_off_1 is not None and end_1 is not None:
                ...
                print(f"🔍 DEBUG: Tiempos manuales encontrados: {match_times}")
                ...
                print(f"🔍 DEBUG: Creado perfil temporal con tiempos manuales y delays: {profile['time_mapping']['delays']}")
                print(f"🔍 DEBUG: Valores del match - global_delay: {match.global_delay_seconds}, event_delays: {match.event_delays}")
        ...
        if profile is None:
            ...
            # Si no hay perfil específico, usar el perfil por defecto
            # Pero solo si no se creó un perfil temporal con tiempos manuales
            if profile is None:
                default_profile = db.query(ImportProfile).filter_by(name='Default').first()
                if default_profile:
                    profile = default_profile.settings
